  ...
  Vantiv::TestCard.all.each do |test_card|
    ...
    context "with a #{test_card.name}" do
      it "returns the same attributes in the live and mocked responses" do
        ...
        live_account_updater = live_response.account_updater
        mocked_account_updater = mocked_response.account_updater
        expect(live_account_updater.payment_account_id).to eq mocked_account_updater.payment_account_id
        expect(live_account_updater.card_type).to eq mocked_account_updater.card_type
        expect(live_account_updater.expiry_month).to eq mocked_account_updater.expiry_month
        expect(live_account_updater.expiry_year).to eq mocked_account_updater.expiry_year
        expect(live_account_updater.extended_card_response_code).to eq mocked_account_updater.extended_card_response_code
      end
